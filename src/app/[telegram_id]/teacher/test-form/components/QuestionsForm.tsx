  ...
  return (
    <div className="mx-auto w-full max-w-3xl p-4 sm:p-6">
      ...
      <div className="grid gap-4">
        {questions.map((q) => (
          ...
          >
              ...
              <button
                ...
                onClick={() => {
                  if (confirm("Bu savolni o'chirishni tasdiqlaysizmi?")) {
                    dispatch(testFromActions.removeQuestion({ question_label: q.question_label }));
                  }
                }}
                className="rounded-md border border-red-300 px-2 py-1 text-xs text-red-700 hover:bg-red-50 dark:border-red-800 dark:text-red-400 dark:hover:bg-red-950/20"
            ...
            <textarea
              ...
              onChange={(e) =>
                dispatch(
                  testFromActions.setQuestionText({
            ...
            {q.question_type === "multiple_choice" && (
              <div className="mt-3 grid gap-2">
                <div className="text-xs font-medium text-neutral-600 dark:text-neutral-400">
                  Variantlar (yashil — to'g'ri javob):
                </div>
                {q.options?.map((opt, idx) => {
                  ...
                  return (
                    <div key={idx} className="flex items-center gap-2">
                      ...
                      >
                        {isCorrect && <span className="text-xs text-white">✓</span>}
                      </button>
                      <input
                        type="text"
                        value={opt}
                        ...
                        className={`flex-1 rounded-md border px-3 py-1.5 text-sm outline-none focus:border-black dark:focus:border-white ${
                          isCorrect
                ...
                {(q.options?.length || 0) < 8 && (
                  <button
                    type="button"
                    onClick={() =>
                      dispatch(testFromActions.addOption({ question_label: q.question_label }))
                    }
                    className="self-start rounded-md border border-dashed border-neutral-300 px-3 py-1 text-xs text-neutral-600 hover:bg-neutral-50 dark:border-neutral-700 dark:text-neutral-400 dark:hover:bg-neutral-800"
                  >
                    + Variant qo'shish
                  </button>
                )}
            ...
            {(q.correct_answer || q.correct_options?.length) && q.question_text && (
              <div className="mt-3">
                <button
                  type="button"
        ...
        <button
          type="button"
          onClick={() => dispatch(testFromActions.addQuestion())}
          className="rounded-lg border-2 border-dashed border-neutral-300 p-4 text-sm font-medium text-neutral-600 hover:border-neutral-400 hover:bg-neutral-50 dark:border-neutral-700 dark:text-neutral-400 dark:hover:bg-neutral-900"
        >
      ...
      {unansweredQuestions.length > 0 && (
        <div className="mt-4 rounded-lg border border-amber-200 bg-amber-50/50 p-4 dark:border-amber-800 dark:bg-amber-950/20">
          <div className="text-sm font-medium text-amber-800 dark:text-amber-300">
            ⚠️ To'liq emas (savol matni yoki javobi yo'q):
          </div>
