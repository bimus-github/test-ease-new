...
export async function getTestAction(testId: string): Promise<{ ok: true; test: TestWithQuestions } | { ok: false; error: string }> {
  if (!isNonEmptyString(testId)) {
    return { ok: false, error: "testId is required" };
  }
  try {
    ...
    console.error("getTestAction error", err);
    return { ok: false, error: "Server error while getting test" };
  }
}
