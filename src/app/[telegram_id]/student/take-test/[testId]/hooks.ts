...
import { checkSubmissionStatusAction, checkSubmissionStatusByUserAndTestAction, getFullSubmissionAction, getTestAction, startSubmissionAction, submitSubmissionAction } from "./actions";
...
import { TAKE_TEST_KEY } from "@/constants/react-query-keys";
import { useParams } from "next/navigation";

export const useGetTest = () => {
  const { testId } = useParams<{ testId: string }>();
  return useQuery<TestWithQuestions | null>({
    queryKey: TAKE_TEST_KEY(testId),
    queryFn: async () => {
      const res = await getTestAction(testId!);
      if (res.ok) return res.test;
